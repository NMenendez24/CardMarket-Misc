...
async function testCRUDOperations() {
  try {
    ...
    const getUsersResponse = await axios.get('http://backend:3000/users');
    console.log('Usuarios obtenidos:', getUsersResponse.data);

    // Crear un nuevo usuario
    const createUserResponse = await axios.post('http://backend:3000/users', {
      nickname: 'Santiago',
    ...
    const getUserResponse = await axios.get(`http://backend:3000/${userId}`);
    console.log('Usuario obtenido por ID:', getUserResponse.data);

    // Actualizar un usuario
    const updateUserResponse = await axios.put(`http://backend:3000/users/${userId}`, {
    ...
    // Eliminar un usuario
    const deleteUserResponse = await axios.delete(`http://backend:3000/users/${userId}`);
